...
public class HandController : MonoBehaviour {
	...
	public static HandModel[] GetHands = new HandModel[2];
	private	Vector3 LeftHandVec;
	private	Vector3 RightHandVec;
  ...
  private long prev_physics_id_ = 0;
	...
	/*private int[] PlayerStates = new int[5];
	private GameObject[] Canvas = new GameObject[5];
	private Vector3[] HandPalms = new Vector3[5];
	*/
  ...
  */
  ...
  void Start() {
	...
	//Change1
	/*
	Canvas[0]=GameObject.Find ("Text1");
	Canvas[1]=GameObject.Find ("Text2");
	Canvas[2]=GameObject.Find ("Text3");
	Canvas[3]=GameObject.Find ("Text4");
	Canvas[4]=GameObject.Find ("Text5");
	*/
      ...
      if (model != null) {
        ...
        if (!all_hands.ContainsKey(leap_hand.Id)) {
          ...
          hand_model.transform.localScale = hand_scale * Vector3.one;
			...
			if(num_hands<=2)
			{
				GetHands[h]=hand_model;
			}
		}
      }
    }
		//checkWinner(HandPalms,all_hands.Count,PlayerStates);
  ...
  */
  ...
  public void ResetRecording() {
	...
	/*private int checkHandState(Vector3 WristPos,Vector3 ThumbPos,Vector3 IndexPos,Vector3 MiddlePos,Vector3 RingPos,Vector3 PinkPos)
	{
		bool checkThumbOpen=false;
		bool checkIndexOpen=false;
		bool checkMiddleOpen=false;
		bool checkRingOpen=false;
		bool checkPinkOpen=false;
		float StateLine = 1.5f;
		int HandState = -1;//-1=unknown 0=rock 2=scissors 5=paper
		if (Vector3.Distance (WristPos, ThumbPos) >= StateLine)
			checkThumbOpen = true;
		if (Vector3.Distance (WristPos, IndexPos) >= StateLine)
			checkIndexOpen = true;
		if (Vector3.Distance (WristPos, MiddlePos) >= StateLine)
			checkMiddleOpen = true;
		if (Vector3.Distance (WristPos, RingPos) >= StateLine)
			checkRingOpen = true;
		if (Vector3.Distance (WristPos, PinkPos) >= StateLine)
			checkPinkOpen = true;
		if (checkThumbOpen && !checkIndexOpen && !checkMiddleOpen && !checkPinkOpen && !checkRingOpen) {
			if (WristPos.y + 1 < ThumbPos.y) {
				Canvas [0].GetComponent<Text> ().text = ("GOOD!!");
				beforestate = 0;
			} else if (WristPos.y - 1 > ThumbPos.y) {
				Canvas [0].GetComponent<Text> ().text = ("BAD!!");
				beforestate = 0;
			} else if (beforestate == 1) {
				Canvas [0].GetComponent<Text> ().text = ("COME ON!!");
				beforestate = 0;
			}
		} else if (!checkThumbOpen && checkIndexOpen && checkMiddleOpen && !checkPinkOpen && !checkRingOpen) {
			Canvas [0].GetComponent<Text> ().text = ("PEACE!!");
			beforestate = 0;
		} else if (checkThumbOpen && checkIndexOpen && checkMiddleOpen && checkPinkOpen && checkRingOpen) {
			beforestate = 1;
		}else if (checkMiddleOpen && checkPinkOpen && checkRingOpen) {
			if (Vector3.Distance (ThumbPos, IndexPos) <= 0.5) {
				Canvas [0].GetComponent<Text> ().text = ("OK!!");
				beforestate = 0;
			}
		} 
			else
	...
	/*private void checkWinner(Vector3[] HandPulmPos,int HandCount,int[] PlayerStateRist)
	{
		if (HandCount >= 2) 
		{
...
int counter=0;
				int PlayerState=2;//0=win 1=lose 2=draw
				for(int j=0;j<HandCount;j++)
				{
					if(j==Player)
						continue;
					if(((PlayerStateRist[Player]==0&&PlayerStateRist[j]==0)||
					   (PlayerStateRist[Player]==2&&PlayerStateRist[j]==2)||
					   (PlayerStateRist[Player]==5&&PlayerStateRist[j]==5))&&
					   (counter==0||PlayerState==2))
					{
						PlayerState=2;
					}
					if(((PlayerStateRist[Player]==0&&PlayerStateRist[j]==2)||
					    (PlayerStateRist[Player]==2&&PlayerStateRist[j]==5)||
					    (PlayerStateRist[Player]==5&&PlayerStateRist[j]==0))&&
					    (counter==0||PlayerState==2||PlayerState==0))
					{
						PlayerState=0;
					}
					if(((PlayerStateRist[Player]==0&&PlayerStateRist[j]==5)||
					    (PlayerStateRist[Player]==2&&PlayerStateRist[j]==0)||
					    (PlayerStateRist[Player]==5&&PlayerStateRist[j]==2))&&
					    (counter==0||PlayerState==2||PlayerState==1))
					{
						PlayerState=1;
					}
				}
				...
				{
				case 0:
					Canvas[Player].GetComponent<Text>().text=("Player"+Player+"=Win");
					break;
				case 1:
					Canvas[Player].GetComponent<Text>().text=("Player"+Player+"=Lose");
					//Debug.Log("Player"+i+"=Lose");
					break;
				case 2:
					Canvas[Player].GetComponent<Text>().text=("Player"+Player+"=Draw");
					//Debug.Log("Player"+i+"=Draw");
					break;
				}
			}
		}
